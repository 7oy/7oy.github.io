...
load(){//инициализация модуля ошибок
	...
	$.in(main,"bar");//создаем контейнер для индикации загрузки и вставляем его в head
	let s=window.location.search;//объект с данными о текущем адресе страницы
	if(s.length==0)$.event(1,$.b,"click",main.cloak);//обработчик для клика на верхней правой части экрана, навешивается для перехода на страницу на которой создаем запись
...
window_decode(text){//функция запроса сообщения и его декодировки
	...
	.then(response=>{
		if("result" in response)m.text=m.a[1].value=response.result;//если есть положительный ответ,записываем полученный результат в текстовое поле
		else if("error" in response)throw new Error(response.error);//если база данных отправила ошибку
	})
	.catch(error=>fail.red(error))//обработка ошибок произошедших на стороне сервера
	.finally(_=>m.wait());//убираем индикатор ожидания
	$.i=0;//указываем, какая форма будет в теле страницы(0 - декодирование, 1 - кодирование)
...
event_encode(){//функция, которая висит на кнопке "кодировать", кодирует и отправляет сообщения на сервер
	let m=this,
		...
		text=m.a[1].value,//текст введённый в форму для кодирования и сохранения на сервер
		message;//зашифрованный текст который будет отправлен на сервер
	if((password.length==0)||(text.length==0))fail.red("Необходимые данные для отправки данных отсутствуют!",m.red(m.a));//проверка на наличие дынных в текстовом поле и в поле пароля
	else{
...
event_decode(e){//при нажатии по кнопке декодирует сообщение
	let m=this,
		a=m.a,
		text;
	if((e.type=="keyup")&&(e.key!="Enter")) return;//если функция вызвана отпусканием кнопки и это не "Enter" то выходим из функции 
	text=(e.ctrlKey)?a[1].value:m.text; /*если нажат "Ctrl", то расшифровываем введённый текст, иначе берем раннее полученные данные с сервера(сделано, чтобы можно было расшифровывать данные не только с сервера)*/
	if((a[0].value.length==0)||(m.text==null))fail.red("Необходимые данные для расшифровки отсутствуют!",m.red(m.a));
	else a[1].value=$.decrypt(a[0].value,text);//декодируем текст с помощь введённого пароля
},
cloak(e){//обработчик клика для вставки формы создания записи, срабатывает если кликнуть мышью в верхнем левом углу, также ширина и длина блока страницы должна быть больше 300 
	let m=this;
	$.event(0,$.b,"click",m.cloak);//удаляем обработчик на вставку формы создания записи
	$.i=1;//указываем, какая форма будет в теле страницы(0 - декодирование, 1 - кодирование)
	m.layout();//вставка макета формы
},
show_link(link){//выводит ссылку для сообщения
	...
	//и второй вариант если страница находится в локальном месте
	m.layout(0);
	$.b.innerHTML="<k>Скопировать ссылку";//вставка контейнера с адресом записи в тело страницы
	$.event(1,$.b.children[0],"click",_=>navigator.clipboard.writeText(actual).catch(e=>fail.red(e)));//копировать в буфер при клике по кнопке(ссылку на запрос информации)
},
layout(i=1){//макет на основе которого создается формы
	let b="<input type=password placeholder=Пароль><textarea placeholder=Текст spellcheck=false></textarea><k>",//поле для ввода пароля и кнопка
		m=this,
		a,
		e=(t,o)=>{//уменьшаем количество кода, t-(a/r) вешаем/стираем событие, o-объект на который вешаем событие
			a=[[t,o[2],"click",[m.event_decode,m.event_encode][$.i]]];//вешаем обработчик на кнопку для кодирования и отправки записи на сервер или декодирования
			!$.i && a.push([t,o[0],"keyup",m.event_decode]);//обработчик отпускания физической кнопки "Enter" в поле ввода пароля
			$.event(...a);
		};
	if(i){
		$.b.innerHTML=b;//вставка содержимого в body
		m.a=$.b.children;//создаём ссылки на элементы
			//0: input - ссылка на поле с паролем
			//1: t	 - ссылка на текстовое поле
			//2: k	 - ссылка на кнопку
		m.a[2].innerText=["Декодировать","Кодировать"][$.i];//обзываем кнопку формы
		e(1,m.a);//вешаем события
	}else e(0,m.a);//снимаем события
},
wait(i=0){$.mod(this.bar,i,"s")},//показывает/скрывает индикатор ожидания(бар/линию загрузки)
...
load(){$.b=$.d.body},
in(obj,element){$.d.head.after(obj[element]=$.d.createElement(element))},//создаем контейнер и вставляем его в head,
...
int_str16(a,r=""){//преобразует массив 8 битных цифр в строку из цифр в 16-й системе
...
async ajax(url,obj=0){//AJAX модуль
	...
	setTimeout(_=>controller.abort(),20000);//таймер на сброс запроса
...
decrypt(password,text){//функция дешифровки зашифрованного текста, путем накладывания на него маски
	...
	for(let i=1,l=arr4_text.length;i<l;i+=2)arr_text.push(parseInt(arr4_text[i-1]+arr4_text[i],16));//превращаем массив символов(1 символ в 16 системе,- 4 бита) в массив 8 битных цифр
...
sha_3(str){//хэш функция SHA3_512
	...
	let r=576,
		c=1024,
		l=c/2,
		s=x=>String.fromCharCode(x),//возвращает символ, получая его код
...
a:["orange","red","green"],//наименования сообщений
load(){//инициализация модуля ошибок
	$.bind_all(fail);
	for(let i=0;i<3;i++)fail[fail.a[i]]=text=>fail.out(text,i);//добавляем в модуль вызов сообщений(по сути это обёртки они одинаковые, но имеют разный цвет и не все удаляются со временем)
	$.in(fail,"alarm");//создаем контейнер для сообщений и вставляем его в head
...
timer(node){//ставит таймер на удаление сообщения
	let timer=setInterval(_=>{
		if(!node.isConnected)clearInterval(timer);//если сообщение уже удалено то удаляем таймер проверки
		if(node.isConnected&&+window.getComputedStyle(this.alarm).order){clearInterval(timer);node.remove()}
/*
	Если сообщение существует и курсор мыши НЕ установлен на блоке сообщений, то удаляем сообщение;
	..считывается свойство CSS(order), которое меняется от наведения курсора, "+" превращает символ в число, так как "0" не преобразуется в 0.
*/
	},5000);
},
delete(){if(event.target.tagName=="X")event.target.parentNode.remove()},//удаляет сообщение по клику
out(text,i){//функция создающая сообщение
	let f=this,//ссылка на данный объект
		a=$.d.createElement(f.a[i]),//создаем элемент для вставки с требуемым тегом
		e;//переменная для хранения ссылки на элемент
	a.innerHTML=text+"<x>";//вставляем в элемент: текст + кнопку закрытия сообщения
	console[["warn","error","info"][i]](text);//дополнительно дублируем в консоль текст сообщения
	e=f.alarm.appendChild(a);////вставляем сообщение в контейнер, и возвращаем ссылку на элемент
	i&&f.timer(e);//чтобы по таймеру объект(сообщение) был удалён(но удаляются не все сообщения для этого и сделана проверка)
}
};
