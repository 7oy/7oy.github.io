...
ingot(password,length){//функция заготовки, создает на основе password и length хеш заданной длины
	...
	for(let i=0;i<length;i++)mask_1+=hash=$.sha_3(password+hash);//рассчитываем первую маску

	for(let i=0;i<length;i++)mask_2+=hash=$.sha_3(password+hash);//рассчитываем вторую маску
...
encrypt(password,text){//функция шифровки текста, путем накладывания на него маски
	...
	let string_16=Array.from(new TextEncoder().encode(text),x=>x.toString(16).padStart(2,"0")).join(""),//строка индексов символов текста
		...
		mask]//кодирующая маска
		=$.ingot(password,n),//получим маски на основе пароля//первая маска, нужна для соединения с текстом, служит для выравнивания длины текста с длиной маски
	...
	h_array[0]=$.sha_3(text);//ключ на основе которого вычисляются остальные хэши
	for(let i=1;i<(n+2);i++){//n+2, - потому, что нужно учитывать ещё два элемента, - ключ и заполнитель
		h_array[i]=$.sha_3(h_array[i-1]);//заполняем массив вычисленными на основе текста(и самих себя) хэшами
	}
	...
	while(h_array[n+1][127-a]==string_16[string_16.length-1]){//проверяем на совпадение символов, крайнего перед вырезаемой строкой из хэша и крайнего символа из текста
		for(let i=0;i<(n+1);i++){//если всё-таки совпали, то удаляем первый хэш(0) и смещаем по позициям остальные хэши
			h_array[i]=h_array[i+1];
		}
		h_array[n+1]=$.sha_3(h_array[n]);//генерируем крайний хэш
	}
...
decrypt(password,text){//функция дешифровки зашифрованного текста, путем накладывания на него маски
	...
	for(let i=1;i<(n+1);i++)h_array[i]=$.sha_3(h_array[i-1]);//заполняем массив полученными на основе ключа хэшами
	for(let i=1;i<(n+1);i++){
